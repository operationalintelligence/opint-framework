...
from opint_framework.apps.example_app.nlp.pyspark_based.utils import convert_endpoint_to_site

test_errors = convert_endpoint_to_site(test_errors)
test_errors.groupby("src_rcsite", "dst_rcsite").count().show(100, truncate=False)

# check nulls
# .filter(test_errors.src_rcsite.isNull() | test_errors.dst_rcsite.isNull())
df = test_errors.filter(test_errors.dst_rcsite.isNull()).toPandas()
null_hostname = set(df.dst_hostname)
df = test_errors.filter(test_errors.src_rcsite.isNull()).toPandas()
null_hostnames = null_hostname.union(set(df.src_hostname))
null_hostnames